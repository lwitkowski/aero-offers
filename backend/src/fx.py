...
def to_price_in_euro(price: Price):
    ...
    if offer_currency and offer_currency != "EUR":
        price_in_euro = str(round(float(str(price.amount)) / exchange_rates[offer_currency], 2))
...
def get_currency_code(currency_str):
    if currency_str in iso_code_mapping:
        ...
        logging.error("Couldn't find currency code for currency symbol: {0}".format(currency_str))
        return currency_str
