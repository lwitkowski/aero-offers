...
def reclassify(db_offer):
    ...
    (manufacturer, model, category) = model_classifier.classify(db_offer['title'], expect_manufacturer, db_offer['page_content'])
    if manufacturer:
        ...
        offers_db.classify_offer(offer_id=db_offer['id'], category=aircraft_type, manufacturer=None, model=None)
