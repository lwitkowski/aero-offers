    ...
    def process_item(self, item: OfferPageItem, _):
        ...
        try:
            ...
            if price is None or price.amount is None:
                msg = "Offer has no valid price, raw_price='{0}' url={1}".format(
                    item.raw_price.strip(), item.url
                )
                self.logger.info(msg)
                raise DropItem(msg)

            if price.amount <= 1:
                msg = "Offer has unreasonable price smaller than 1, price={0}, url={1}".format(
                    price.amount_text, item.url
                )
                self.logger.info(msg)
                raise DropItem(msg)

            if price.amount > 500_000:
                msg = "Offer has unreasonable price higher than 500_000, price={0}, url={1} ".format(
                    price.amount_text, item.url
                )
                self.logger.info(msg)
                raise DropItem(msg)
